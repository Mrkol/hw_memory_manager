...
template<std::size_t PAGE_SIZE>
char* StackAllocatorImpl<PAGE_SIZE>::allocate(std::size_t count)
{
	if (count > PAGE_SIZE)
	{
		return nullptr;
...
template<typename T, std::size_t PAGE_SIZE>
auto StackAllocator<T, PAGE_SIZE>::allocate(std::size_t count) -> pointer
{
	return reinterpret_cast<pointer>(impl_->allocate(count * sizeof(value_type)));
}

template<typename T, std::size_t PAGE_SIZE>
void StackAllocator<T, PAGE_SIZE>::deallocate(pointer ptr, std::size_t count)
{
